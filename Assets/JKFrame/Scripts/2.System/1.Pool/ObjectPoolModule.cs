...
namespace JKFrame
{
    public class ObjectPoolModule
    {
        ...
        public void InitObjectPool<T>(string keyName, int maxCapacity = -1, int defaultQuantity = 0) where T : new()
        {
            ...
            if (poolDic.TryGetValue(keyName, out ObjectPoolData poolData))
            {
                ...
                if (defaultQuantity != 0)
                {
                    ...
                    //判断更新容器最后的长度比之前的宽度
                    if (defaultQuantity - nowCapacity >= 0)
                    {
                        //生成差值容量个数的物体放入对象池
                    ...
                    {
                        DelObjectLen(keyName, Mathf.Abs(defaultQuantity - nowCapacity));
                    }
                }
            }
            //设置的对象池不存在
        ...
        //=================================================
        public bool DelObjectLen(object obj, int len)
        {
            return DelObjectLen(obj.GetType().FullName, len);
        }
        public bool DelObjectLen(string keyName, int len)
        {
            if (poolDic.TryGetValue(keyName, out ObjectPoolData poolData) == false)
            {
                return false;
            }
            return poolData.DelObj(len);
        }
        public void ClearAll()
        {
            ...
            {
                enumerator.Current.Value.Destory(false);
            }
            poolDic.Clear();
        }
